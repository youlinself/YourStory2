...
const OutlineView: React.FC<OutlineViewProps> = ({
  ...
  return (
    <div className="p-4">
      <h3 className="text-subheading mb-3">自传大纲</h3>
      ...
      <div className="space-y-1">
        {chapters.map((chapter, index) => {
          ...
          return (
            ...
            >
              <span className="flex items-center gap-2">
                <span className={`${status.color} text-base leading-none`}>{status.icon}</span>
      ...
      <button
        ...
        className="w-full mt-2 px-3 py-2 rounded-lg text-xs text-ink-muted hover:text-ink-secondary hover:bg-bg-secondary transition-colors"
      >
        查看完整自传 →
      </button>
    </div>
